...
import type { LocalResolutionSeriesProjection } from "../../state/projections";
...
interface NumericForecastCardProps {
  ...
  marketSeries: LocalResolutionSeriesProjection;
...
function Sparkline({
  ...
  points: LocalResolutionSeriesProjection["points"];
...
export function NumericForecastCard({
  ...
  const latest = marketSeries.points.at(-1) ?? null;
  const earliest = marketSeries.points.at(0) ?? null;
  const window =
  ...
  const windowPoints = window
    ? marketSeries.points.filter((point) => point.date >= window.start_date && point.date <= window.end_date)
    : [];
  ...
  const questionLabel = question?.metadata?.question_kind === "brent_weekly_high"
    ? "Brent weekly high"
    : marketSeries.label;
  const sourceNote = marketSeries.seriesId === "DCOILBRENTEU"
    ? "现货说明：DCOILBRENTEU 为 ICE Brent 现货日价，非期货；ICE M1 期货通常与现货差 ±$1–3/bbl。"
    : `本地数据说明：${marketSeries.source} 来自 data/normalized/market/fred_series.csv；UI 不联网拉取。`;
  ...
  return (
    <section className="console-card numeric-forecast-card">
      ...
      <div className="numeric-forecast-answer">
        <span>{questionLabel} · {marketSeries.unit} · {marketSeries.source}</span>
        <em>{runtimeLabel}</em>
        <strong>{formatPrice(prediction)} <small>{marketSeries.unit}</small></strong>
        <p>分辨率窗口 · {window ? `${window.start_date} → ${window.end_date}` : "目标窗口待定"}</p>
      </div>
      <div className="numeric-forecast-chart">
        <Sparkline points={marketSeries.points} prediction={prediction} />
      ...
      <div className="numeric-forecast-source">
        <span><Target size={14} /> 图示范围 {sparklineRange}（{marketSeries.points.length} 日）</span>
        <span><Activity size={14} /> {marketSeries.source} 抓取 {marketSeries.retrievedAt || "待定"}</span>
        <span className="numeric-forecast-note">{sourceNote}</span>
      </div>
